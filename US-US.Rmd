...
ggplot(CPPI_plot_df, aes(x = Date, y = Value, color = Type)) +
  ...
  labs(
    ...
    subtitle = "Comparison between Portfolio 1 and European Equity",
    x = "Date",
    y = "Guarantee",
    color = "Strategy"
  ) +
  ...
  scale_color_manual(
    values = c(
      "European Equity" = "#0072B2",     # blue
      "Portfolio 1" = "#E41A1C",         # red
      "Initial Guarantee" = "#009E73"    # green
    ),
...
ggplot(ef, aes(x = SD, y = Return)) +
  ...
  geom_text(data = gmv_point, aes(label = Label), nudge_x = -0.002, nudge_y = 0.001, size = 2.3, fontface = "bold") +
...
points_df$Label[2:6] <- paste("Portfolio", 1:5) # Portfolios 1-5 are the intermediate points
...
ggplot(ef_efficient, aes(x = SD, y = Return)) +
  ...
  geom_text(data = points_df, aes(label = Label),
            nudge_y = 0.0005, nudge_x = -0.002, size = 2, vjust = -0.5, fontface = "bold") +
  labs(x = expression(sigma), y = expression(mu)) +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_blank(),
...
p <- ggplot(All_years, aes(x = TIME_PERIOD, y = z_1m)) +
  ...
  labs(
    ...
    subtitle = "Yearly rates on a monthly basis",
    x = "Time",
    y = "Risk-Free Rate (%)"
  ) +
  theme_minimal(base_size = 14) +
