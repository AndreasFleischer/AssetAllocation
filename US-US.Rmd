...
P1returns <- readRDS("returns_choice.rds")
P1 <- CPPI(1,125/100,130/100, P1returns)
CPPI_plot_df <- data.frame(cbind(P1returns$date[1:89],cbind(EEandIG,P1[,1]))) %>% pivot_longer(cols = -"X1", values_to = "value", names_to = "type")


ggplot(data = CPPI_plot_df, mapping = aes(x = ym(X1), y = value, colour = type)) + geom_point()
```
# CPPI plot
