...
ggplot(ef, aes(x = SD, y = Return)) +
  ...
  geom_point(data = gmv_point, aes(x = SD, y = Return), color = "#D55E00", size = 3.5) +
  geom_point(data = max_point, aes(x = SD, y = Return), color = "#0072B2", size = 3.5) +
  geom_text(data = gmv_point, aes(label = Label), nudge_x = -0.001, nudge_y = 0.001, size = 3, fontface = "bold") +
  geom_text(data = max_point, aes(label = Label), nudge_x = -0.002, nudge_y = 0.002, size = 3, fontface = "bold") +
  labs(
    x = expression(sigma),
    y = expression(mu)
  ) +
...
ggplot(ef_efficient, aes(x = SD, y = Return)) +
  ...
  geom_point(data = points_df, aes(x = SD, y = Return, color = Color), size = 3.5) +
  scale_color_identity() + # Use the explicit color values in the 'Color' column
  # Plot labels with bold font
  geom_text(data = points_df, aes(label = Label),
            nudge_y = 0.001, nudge_x = -0.002, size = 3, vjust = -0.5, fontface = "bold") +
  labs(x = expression(sigma), y = expression(mu)) +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_blank(),
