...
variance <- c(var_func(w_min_var, Sigma, k))
for(i in 1:n_internal){
  variance <- c(variance, var_func(w_internal[i,,], Sigma, k))
}
variance <- c(variance, var_func(w_max_return, Sigma, k))

# returns 
return_min_var <- sum(w_min_var[k,]*mu[k,])
return_max_return <- sum(w_max_return[k,]*mu[k,])
return_internal <- rep(NA, n_internal)
for(i in 1:n_internal){
  return_internal[i] <- sum(w_internal[i,k,]*mu[k,])
}
return <- c(return_min_var, return_internal, return_max_return)
return
