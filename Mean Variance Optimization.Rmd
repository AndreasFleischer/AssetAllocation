...
excess_returns_US <- sweep(size_mom_6_US[,-1], 1, RF.US$RF, "-") %>% 
  mutate(.before = 1, date = size_mom_6_US$date)

size_mom_6_monthly <- left_join(excess_returns_EU, excess_returns_US, by = "date", suffix = c(".EU", ".US"))
...
names(w_df3) <- names(size_mom_6_monthly[, -1])
w_df3 <- w_df3 %>% 
  mutate(.before = 1, date = size_mom_6_EU$date[36:nrow(size_mom_6_EU)])
round(w_df3, 3)
