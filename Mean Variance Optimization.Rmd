...
<!-- ```{r} -->
<!-- optimize_portfolio <- function(excess_returns, cov_period, alpha){ -->
<!--   n_assets <- length(excess_returns) -->
...
<!-- optimize_internal <- function(excess_returns, cov_period, target_return, alpha){ -->
<!--   n_assets <- length(excess_returns) -->
...
<!-- optimize_return <- function(excess_returns, alpha){ -->
<!--   n_assets <- length(excess_returns) -->
<!--   objective.in <- excess_returns -->
...
<!--     mu[i,] <- colMeans(returns_period) -->
<!--     Sigma[i,,] <- cov(returns_period) -->
<!--       w_min_var[i,] <- optimize_portfolio(mu[i,], Sigma[i,,], alpha) -->
<!--       w_max_return[i,] <- optimize_return(mu[i,], alpha) -->
...
<!--       target_returns[i,] <- target_returns_tmp[-c(1, length(target_returns_tmp))] -->
<!--       for(j in 1:length(target_returns[i,])){ -->
<!--         w_internal[j,i,] <- optimize_internal(mu[i,], Sigma[i,,], target_returns[i,j], alpha) -->
<!--       } -->
<!--   } -->
<!--   w = array(NA, c(7, dim(w_min_var)[1], dim(w_min_var)[2])) -->
<!--   w[1,,] <- w_min_var -->
...
<!--   for(i in 1:dim(w)[[1]]){ -->
<!--     returns_out_of_bag[,i] <- rowSums(w[i,,]*factors_out_of_bag[,2:7]) + (1-rowSums(alpha*w[i,,c("SMB_US","MOM_US", "SMB_EU", "MOM_EU")]))*factors_out_of_bag[,"RF"] -->

<!--     returns_in_bag[,i] <- rowSums(w[i,,]*mu) + (1-rowSums(alpha*w[i,,c("SMB_US","MOM_US", "SMB_EU", "MOM_EU")]))*returns[36:(nrow(returns)-1),"RF"] -->
<!--   } -->
...
<!-- var_short <- rep(NA, 208) -->
<!-- for(i in 1:208){ -->
<!--   var_short[i] <- t(h$w[2,i,])%*%h$Sigma[i,,]%*%h$w[2,i,] -->
<!-- } -->
