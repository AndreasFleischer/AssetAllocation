...
<!-- ```{r Til opgave 1b} -->
<!-- returns_b <- factors %>% filter(date >= 200409 & date <= 202412) -->
<!-- returns_b <- cbind(returns_b, RF.EU %>% filter(TIME_PERIOD >= ym(200409) & TIME_PERIOD <= ym(202501)) %>% select(return)) -->
<!-- target_returns_b <- readRDS("target_returns_portfolio_1.rds")+0.5 -->

<!-- n_assets <- ncol(returns_b[,-1]) -->
<!-- n <- nrow(returns_b) - 35 - 1 -->
<!-- w <- matrix(NA, nrow = n, ncol = n_assets) -->
<!-- Sigma <- array(NA, c(n, n_assets, n_assets)) -->
<!-- mu <- array(NA, c(n, n_assets)) -->

<!-- for(i in 1:n){ -->
<!--   excess_returns_period <- returns_b[i:(i+35),-1] -->
<!--   mu[i,] <- colMeans(excess_returns_period) -->
<!--   Sigma[i,,] <- cov(excess_returns_period) -->
<!--   w[i,] <- optimize_internal2(mu[i,], Sigma[i,,], target_returns_b[i]) -->
<!-- } -->


<!-- colnames(w) <- colnames(returns_b[,-1]) -->
<!-- factors_period <- returns_b[37:nrow(returns_b),] -->
...
all_factors <- data.frame(date = factors$date,
                          ...
                          RF = RF.EU$return)
returns_b <- all_factors %>% filter(date >= 200409 & date <= 202412)

cov_period <- cov(returns_b[,-1])
n_assets <- length(colMeans(returns_b[,-1]))
target_variance <- 7 
...
optimize_b <- function(mean_returns, cov_period, target_variance){
  ...
  lower_bounds <- c(rep(-1000, n_assets-1), 0)
  upper_bounds <- rep(1000, n_assets)
...
optimal_weights <- optimize_b(colMeans(returns_b[,-1]), cov(returns_b[,-1]), 10)
...
for(i in 1:n){
  returns_b_period <- returns_b[i:(i+35),-1]
  mu_b[i,] <- colMeans(returns_b_period)
  Sigma_b[i,,] <- cov(returns_b_period)
