...
## Efficient frontier
```{r}
n_assets <- ncol(factors[,-1])
n <- nrow(factors) - 35 - 1
w_matrix_min_var <- matrix(NA, nrow = n, ncol = n_assets)
w_matrix_max_return <- matrix(NA, nrow = n, ncol = n_assets)

n_internal <- 10
w_matrix_internal <- array(NA, dim = c(n_internal, n, n_assets))
target_returns <- array(NA, c(n, n_internal))

for(i in 1:n){
  excess_returns_period <- factors[i:(i+35),-1]
  mu <- colMeans(excess_returns_period)
  cov_period <- cov(excess_returns_period)
  if(all(mu <= 0)){
    w_matrix_min_var[i,] <- 0
    w_matrix_max_return[i,] <- 0
    w_matrix_internal[,i , ] <- 0
    target_returns[i,] <- 0
  } else{
    w_matrix_min_var[i,] <- optimize_portfolio(mu, cov_period)
    w_matrix_max_return[i,] <- optimize_return(mu)
    
    return_min_var <- sum(mu*w_matrix_min_var[i,])
    return_max_return <- sum(mu*w_matrix_max_return[i,])
    ...
    for(j in 1:length(target_returns[i,])){
      w_matrix_internal[j,i,] <- optimize_internal(mu, cov_period, target_returns[i,j])
    }
  }
}

colnames(w_matrix_min_var) <- colnames(factors[,-1])
current_dimnames <- dimnames(w_matrix_internal)
current_dimnames[[3]] <- colnames(factors[,-1])
dimnames(w_matrix_internal) <- current_dimnames
colnames(w_matrix_max_return) <- colnames(factors[,-1])

round(w_matrix_internal[,1,], 3)
round(target_returns[1,], 3)
```
