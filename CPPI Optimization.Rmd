...
### target
```{r}
guarantees_target <- map(guarantees_trigger_target, "Guarantee")
...
### Trigger
```{r}
# guarantees_trigger_target <- readRDS("C:/Users/nikolaj/OneDrive - University of Copenhagen/Aktuar/5. år/Asset allocation/Exam project/CPPI_strats_m_2.5")

guarantees_trigger <- map(guarantees_trigger_target, "Guarantee")
...
```
##### Effect for larger values of target
```{r}
# m <- seq(1, 20, 0.5)
m <- 2.5
L_target <- 1.27
...
```{r "m = 2.5 target = 127% trigger = 134%"}
m <- 2.5
L_target <- 127/100
L_trigger <- 134/100
returns_choice <- readRDS("returns_choice.rds")
...
values = c(
  "Portfolio.1" = colors_pal[2], 
  "European.Equity" = "#009E73", 
  "Portfolio.1.Recommended" = "orange"
)
...
guarantee_df %>%
  ...
  ggplot(aes(x = as.Date(date), y = Guarantee, color = Portfolio)) +
    ...
    scale_y_continuous(labels = scales::number_format(accuracy = 0.01)) +
    labs(
      title = "Development of recommended CPPI Guarantees",
      #subtitle = "Guarantee level implied by CPPI floor mechanism",
      x = "Start date of TDF",
      y = "Guarantee"
    ) +
...
```
## recommended CPPI strat with risk parity
```{r}
